package tests;

import org.junit.Test;
import org.junit.jupiter.api.Assertions;

import entities.Pneus;


public class PneuTeste {

		@Test
		public void testRemoveStock() {
			Pneus p1 = new Pneus("XXXX", "XXXXX", 10, 10 ,10, 10.50);
			p1.setStock(10);
			p1.removeStock(1);
			Assertions.assertEquals(9,p1.getStock());
			p1.removeStock(2);
			Assertions.assertEquals(7,p1.getStock());
			p1.addStock(1);
			Assertions.assertEquals(8,p1.getStock());
	}
		@Test
		public void testAddCart() {
			Pneus p1 = new Pneus("XXXX", "XXXXX", 10, 10 ,0, 10.50 );
			p1.setCart(0);
			p1.addCart("10");
			Assertions.assertEquals(10,p1.getCart());
			
	}
		@Test
		public void testRemoveCart() {
			Pneus p1 = new Pneus("XXXX", "XXXXX", 10, 10 ,0, 10.50 );
			p1.removeCart("10");
			Assertions.assertEquals(0,p1.getCart());
			p1.setCart(10);
			p1.removeCart("0");
			Assertions.assertEquals(10,p1.getCart());
			p1.setCart(10);
			p1.removeCart("-10");
			Assertions.assertEquals(0,p1.getCart());
   }
		@Test 
		public void testAddStock(){
			Pneus p1 = new Pneus("XXXX", "XXXXX", 0, 0 ,0, 10.50 );
			p1.addStock(10);
			Assertions.assertEquals(10,p1.getStock());
			p1.addStock(0);
			Assertions.assertEquals(11,p1.getStock());
			p1.addStock(-1);
			Assertions.assertEquals(12,p1.getStock());
	}
}
